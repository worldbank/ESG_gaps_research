...
This report uses a different selection of ESG indicators than that used in the 2018 report.
...
indicators from two products introduced since the 2018 report: the Blackrock Sovereign Sustainability
Index (BSSI) and the World Bank's own [curated ESG dataset][esg-ddh]. Further research into the original
group of data providers has produced additional indicators that have been included. Conversely, we decided
to remove a set of indicators used exclusively by Versk-Maplecroft because the strong similarities
among them (e.g., very similar trade or debt measures) were resulting in double-counting that could
potentially skew the findings. Accordingly, this report is based on a body of 137 indicators, compared
to 134 in the 2018 report.

The other major difference between the indicators of the two reports is that many of them have been updated
since the 2018 report was completed. Many statistical indicators have been updated several times. Accordingly, if the 2018
analysis were re-run using the indicators from this analysis (which date from October, 2019), the findings would likely be quite different,
