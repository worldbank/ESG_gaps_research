...
Interviews ESG data providers found that most obtain at least some, and often a subsantial amount
of, ata from World Bank databases.
137 indicators were specifically identified from these interviews, of which 127 could be mapped to
active databases in theWorld Bank's data API, enabling the authors to [perform a rapid
...
This report uses a different selection of ESG indicators than that used in the 2018 report: 130 indicators
compared to the original 137, but not excluding indicators in inactive databases as this is an important consideration
in the gaps analyis.
The sample of indicators has expanded to include additions from two new sources: the
forthcoming Blackrock Sovereign Sustainability Index (BSSI) and the World Bank's own [curated ESG dataset][esg-ddh] (4 indicators
from the latter source were not included because they are modeled projections that are not relevant to this study).
...
similarities between them (e.g., groups of very similar trade or debt measures) may have been skewing the results.

The other factor that has changed since the 2018 report is of course that it is now a year later, and some underlying
databases have been updated (at least once) with new data, while others have not been updated. As a result, if the same
analysis were peformed today with 2019 as a reference the results would likely be different than those above. That said,
