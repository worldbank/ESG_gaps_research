...
This paper analyzed a set of 123 ESG investment related indicators
sourced from the World Bank's data catalog
in order to provide a comprehensive overview of the sources, timeliness and statistical methods through which the selected indicators have been created. 
We have identified a high number of indicators, 26, that are no longer updated in the WDI database or that do not belong to the WDI database.
(( Whether an indicator is in WDI--as opposed to some other WBG database--is not really relevant ))
The social indicators appear to be the most robust indicators having the least number of indicators that come with missing data on key variables of interest for this paper, followed by governance and environmental indicators. 
...
(( this this a fair inference? I've seen some pretty sparse data coming from intl orgs, including the Bank ))
...
(( need to updaet this to reference the existing ESG framework ))


# Introduction

This document aims at providing an overview of the indicators curated and disseminated by the World Bank Group that are relevant for ESG (Environmental, Social, Governance) investment. The work has been conducted by assembling a metadata database on indicators, interviewing data managers, and analyzing the set of 123 identified indicators
(( you need to inform reader as to how these indicators were identified -- see 2018 options paper ))
and their corresponding metadata. We start with a detailed overview of the methodology used to analyze the dataset and we continue with providing a set of insights on sources, processing entities and dissemination of these indicators and the frequency of their updates and release.  
...
(( correction: this classification should come from the 2018 options paper as an input to this research ))
