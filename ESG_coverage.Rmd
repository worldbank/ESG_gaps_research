...
*TGH: How is the chart sorted? The description makes it sound like is sorted by descending coverage, but it doesn't look like it*

```{r ciavailability, fig.width = 12, fig.height = 10, fig.cap = "Number of countries per indicator over time"}
pg1
```

The characteristics of ESG indicator coverage vary significantly across the study area, but can be generally classified under one
of five different patterns:

*TGH: a couple of overall observations on 2.1 - 2.5. 1) 2.4 is by far the most important category, as MRV is now we defined a "data gap" in
the introduction. Would be good to highlight this and perhaps elaborate more; 2) I'm not getting a detailed picture from the discussion yet,
and I wonder if we could include a list in each section of the indicators that fit that section, absent that, a count or percentage of how
many indicators are in each section. Might also think about a computed "coverage score" we could calculate for each indicator. Note that the
indicators classified under 2.5 (High coverage) should match the "no gaps" classification in the next section.*
...
*TGH: no longer relevant since the chart starts at 2000*
