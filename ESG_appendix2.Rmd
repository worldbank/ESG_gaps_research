...
This appendix includes a range of options and definitions for classifying indicator coverage
as discussed in the section on [coverage analysis](coverage.html).
Throughout our research we discussed several options for thinking about clusters of indicators,
and the complete list, along with their formulas and definitions, are included here for reference.
Several of these approaches have strong similarities to the ones that appear in the coverage analysis
section, but with slightly different or more precise definitions that may be useful in future research.
...
```{r dtlc}
  ...
  report.datatable(d4, c("Code", 
                 "Indicator", 
                 "Average of countries per year"),
                 "Average Countries per Indicator",
                 precision=list(mean=1),
                 pdf=list(
                   widths=list(12, 25, 10))
                 )
...
report.datatable(lmdi, c("Code", 
                 "Indicator", "Avg. growth in coverage (penalized)", 
                 "No. of years with data"),
                 "Average Coverage Growth Per Indicator",
              precision=list(beta=1),
              pdf=list(widths=list(12, 25, 5, 5))
            )
```
...
```{r dtsi}
  ...
  report.datatable(si,
    c("Code", "Indicator", "Avg. of countries", "Standard Dev."),
    "Indicator Stability Over Time",
    precision=list(mean=1, sd=2),
    pdf=list(widths=list(12, 25, 5, 5)))
```
...
```{r sdd}
  ...
  report.datatable(sdd,
    c("Code", "Indicator", "Year", "No. Countries dropped"),
      "Coverage Decline Per Indicator",
      pdf=list(widths=list(12, 25, 5, 5))
    )
```
...
```{r icli}
  ...
  report.datatable(ici,
    c("Code", "Indicator", "No. Years covered", "No. intervals with no data (intermittentness)", "Avg. coverage interval", "Lumpiness index"),
    "Indicators and 'Lumpy' Coverage",
    precision=list(aci=1, li=2),
    html=list(
      args=list(
        columnDefs=list(
          list(width = '380px', targets = 1),
          list(width = '35px', targets = c(2:5))
        )        
      )
    ),
    pdf=list(widths=list(12, 10, 5, 5, 5, 5, 5))
    )
