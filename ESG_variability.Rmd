...
Note that the scope of this paper is limited to analysis of the *potential* for use of statistical
...
the availability of proxies and independent variables (for modelling) and other ideosyncratic factors. 
...
Note that indicator/country series consisting of a single year are not included in \@ref(fig:civar) since there is
no "variation" in a domain of just one observation.
...
Figure \@ref(fig:esg-var) is a very dense display of data, but its advantage is that it makes it easy
...
```{r esg-var-percs2, fig.cap = "Density of of ESG indicator CV", fig.margin = FALSE, fig.width = 6}
...
One important feature of \@ref(tab:esg-var-percs) and \@ref(fig:esg-var-percs2) is the presence of a substantial number
...
Figure \@ref(fig:esg-var) shows the distribution of mean CV values for the ESG indicator dataset. Again, the the red
line marks the median, while the blue lines mark the 5th and 95th percentiles.
...
to extrapolate in various combinations, as shown by Table \@ref(tab:cv-estimates).
