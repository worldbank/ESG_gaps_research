...
diff_ind %>%
  ...
  datatable(colnames = c("Indicator", "Country", "Max", "Country", "Min", "Diff"),
            ...
            options = list(pageLength = 6,
                           autowidth = TRUE,
                           columnDefs = list(list(width = '40px', targets = c(2, 4, 5)),
...
p_irank  <- ggplotly(chbar(df = filter(ialy, lst_yd  > 0)), tooltip = c("text")) %>% plotly::style(hoveron = "fill")
p_irank2 <- ggplotly(chbar(df = filter(ialy2, lst_yd > 0)), tooltip = c("text")) %>% plotly::style(hoveron = "fill")
